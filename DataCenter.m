...
% phenomena we expected. We see the instability in the frequency around
...
% Iterate through each axes object and set the FontSize for labels and ticks
for i = 1:length(allAxes)
    set(allAxes(i), 'FontSize', 30); % Sets font size for tick labels
    set(get(allAxes(i), 'XLabel'), 'FontSize', 30); % Sets font size for X-axis label
    set(get(allAxes(i), 'YLabel'), 'FontSize', 30); % Sets font size for Y-axis label
    set(get(allAxes(i), 'Title'), 'FontSize', 30); % Sets font size for title

    % Get all line objects within the current axes
    allLines = findall(allAxes(i), 'type', 'line');
    % Iterate through each line object and set the LineWidth
    for j = 1:length(allLines)
        set(allLines(j), 'LineWidth', 2); % Sets line width to 2
    end

    % Turn on grid for the current axes after creation
    set(allAxes(i), 'XGrid', 'on', 'YGrid', 'on');
end
